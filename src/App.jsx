...
import { 
  ...
  ChevronRight, MapPin, Copy, Activity
} from 'lucide-react';

// REEMPLAZA CON TUS DATOS DE FIREBASE SI LOS TIENES
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-200 p-4 font-sans">
      <header className="max-w-4xl mx-auto flex justify-between items-center mb-10 pb-6 border-b border-white/5">
        ...
        <button onClick={() => setShowAdmin(!showAdmin)} className="p-2 bg-slate-900 rounded-xl border border-white/10">
      ...
      <main className="max-w-4xl mx-auto">
        {!showAdmin ? (
          <div className="space-y-6">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-slate-900 border-2 border-white/5 rounded-3xl py-6 pl-16 pr-8 text-xl font-black uppercase italic focus:border-amber-500 outline-none"
                value={searchTerm}
              ...
              {filteredItems.length > 0 && !selectedSite && (
                <div className="absolute top-full left-0 w-full mt-2 bg-slate-900 border border-white/10 rounded-2xl shadow-2xl z-50 overflow-hidden">
                  {filteredItems.map(item => (
                    <button key={item.id} onClick={() => { setSelectedSite(item); setSearchTerm(''); }} className="w-full px-8 py-4 flex justify-between items-center hover:bg-amber-500 group">
            ...
            {selectedSite && (
              <div className="bg-slate-900 rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl">
                <div className="p-8 bg-amber-500 text-black">
                  ...
                  <p className="text-xl font-bold opacity-70">{selectedSite['Código'] || selectedSite.Codigo}</p>
                </div>
                <div className="p-8 grid gap-6">
                    <div className="p-6 bg-slate-950 rounded-2xl">
                        <p className="text-[10px] font-black text-slate-500 uppercase mb-2 tracking-widest">Dirección</p>
                        <p className="font-bold uppercase text-lg">{selectedSite.Dirección || 'S/D'}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 bg-slate-950 rounded-2xl text-center">
                            <p className="text-[9px] font-black text-slate-500 uppercase mb-1">Distrito</p>
                            <p className="font-bold">{selectedSite.Distrito || '---'}</p>
                        </div>
                        <div className="p-4 bg-slate-950 rounded-2xl text-center">
                            <p className="text-[9px] font-black text-slate-500 uppercase mb-1">Depto</p>
                            <p className="font-bold">{selectedSite.Departamento || '---'}</p>
                        </div>
                    </div>
                </div>
                <div className="p-6 bg-slate-950/50 text-center">
                    <button onClick={() => setSelectedSite(null)} className="px-8 py-2 bg-slate-800 rounded-full text-[10px] font-black uppercase tracking-widest">Cerrar</button>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-slate-900 p-10 rounded-[2.5rem] border border-white/10 text-center">
            <h2 className="text-2xl font-black uppercase italic text-amber-500 mb-6 flex justify-center gap-3"><Database /> Panel de Carga</h2>
            <div className="grid md:grid-cols-2 gap-6">
                <label className="flex flex-col items-center justify-center border-2 border-dashed border-white/10 rounded-3xl p-10 hover:border-amber-500 cursor-pointer">
                    {importStatus.loading ? <Loader2 className="animate-spin text-amber-500" /> : <UploadCloud size={40} className="mb-4 text-slate-600" />}
                    <p className="text-[10px] font-black uppercase">Subir CSV</p>
                    <input type="file" className="hidden" accept=".csv" onChange={handleCSVImport} disabled={importStatus.loading} />
                </label>
                <div className="bg-slate-950 p-10 rounded-3xl">
                    <p className="text-5xl font-black text-white mb-2">{items.length}</p>
                    <p className="text-[10px] font-black uppercase text-emerald-500 tracking-widest">Sitios Activos</p>
                </div>
            </div>
          </div>
        )}
